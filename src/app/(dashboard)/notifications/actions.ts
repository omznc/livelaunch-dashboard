...
export const updateFilters = async (guildId: string, settings: NotificationsFilterSettings): Promise<void> => {
...
export const addCountdown = async (guildId: string, settings: CountdownSetting): Promise<void> => {
  const authorized = await isAuthorizedForGuild(guildId);
  if (!authorized) {
    throw new Error('Unauthorized');
  }
...
export const removeCountdown = async (guildId: string, minutes: number): Promise<void> => {
  const authorized = await isAuthorizedForGuild(guildId);
  if (!authorized) {
    throw new Error('Unauthorized');
  }
...
export const updateChannel = async (guildId: string, channelId: string): Promise<void | string> => {
  ...
  if (guild?.notification_webhook_url) {
    await rest
      .delete(Routes.webhook(guild.notification_webhook_url.split('/')[5]))
      // cleanup on error
      .catch(async e => {
        await prisma.enabled_guilds.update({
          where: {
